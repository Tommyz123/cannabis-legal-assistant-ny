...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function sendMessage() {
        ...
        try {
            ...
            hideTypingIndicator();
            appendMessage('ai', `<span class="text-red-400">连接失败，请检查后端服务是否运行。(${e.message})</span>`);
        }
    }
    ...
    const quickActionQueries = {
        '查询法规': '纽约州大麻零售许可证有哪些要求？',
        '合规检查': '请帮我检查大麻产品包装的合规性要求。',
    };

    quickActionBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            const actionTitle = btn.querySelector('h3').innerText;
            const query = quickActionQueries[actionTitle] || `${actionTitle}相关法规查询`;
            welcomeScreen.classList.add('hidden');
    ...
    function renderAIResponse(data) {
        ...
        if (data.sources && data.sources.length > 0) {
            ...
            html += `<p class="text-xs font-orbitron text-gray-500 uppercase tracking-widest mb-2">参考来源</p>`;
        ...
        if (data.warnings && data.warnings.length > 0) {
            ...
            html += `<p class="text-xs font-orbitron text-yellow-500 uppercase tracking-widest mb-2">合规提醒</p>`;
